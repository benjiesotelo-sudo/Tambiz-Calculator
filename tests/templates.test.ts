...
import { ADVISER_COLUMNS, buildTemplate, parseWorkbook, ROLL_COLUMNS, TEMPLATES } from '@/lib/excel-import';

const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, '');

describe('download templates', () => {
  for (const [kind, template] of Object.entries(TEMPLATES)) {
    it(`the ${kind} template has every column of the importer, in its order, and imports as its one example row`, async () => {
      const buf = await buildTemplate(template);
      const wb = new ExcelJS.Workbook();
      await wb.xlsx.load(buf as unknown as ArrayBuffer);
      const headings: string[] = [];
      wb.worksheets[0].getRow(1).eachCell((c) => headings.push(String(c.value)));
      expect(headings).toEqual(template.columns.map((c) => c.header));

      const parsed = await parseWorkbook(buf as unknown as ArrayBuffer, template.columns, kind);
      expect(parsed.problems).toEqual([]);
      expect(parsed.rows.map((r) => [r.sheet, r.values])).toEqual([[template.sheet, Object.fromEntries(template.columns.map((c) => [c.field, c.example]))]]);
    });
  }
